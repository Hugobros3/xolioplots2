...
public class PlotsWorldGenerator extends BlankWorldGenerator {
	...
	public Chunk generateChunk(Chunk chunk) {
		...
		for (int x = 0; x < 32; x++)
			for (int z = 0; z < 32; z++)
			{
				...
				{
					chunk.setVoxelDataWithoutUpdates(x, y % 32, z, 3);
				}
				...
				if(GROUND_HEIGHT >= cy * 32  && cy * 32 + 32 > GROUND_HEIGHT)
				{
					...
					chunk.setVoxelDataWithoutUpdates(x, GROUND_HEIGHT % 32, z, topTile);
				}
			}
		return chunk;
	}
